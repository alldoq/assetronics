    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-primary-dark">Edit asset</h1>
            <p class="text-slate-500 mt-1">Update the asset details</p>
          </div>
      ...
      <!-- Loading state -->
      <div v-if="isLoadingAsset" class="bg-white border border-border-light rounded-2xl p-8 text-center">
        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-accent-blue mx-auto mb-4"></div>
        <p class="text-slate-600">Loading asset...</p>
      </div>
      ...
      <!-- Main Content -->
      <div v-if="!isLoadingAsset" class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3">
          <form @submit.prevent="handleSubmit">
              ...
              <div class="space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    class="input-refined"
                    placeholder="e.g., Apple, Dell, Amazon"
                  />
                </div>
              </div>
            </div>
              ...
              <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div>
                    ...
                    <select id="department" v-model="form.department" class="input-refined">
            ...
            <div class="mt-6 flex justify-end gap-3">
              ...
              >
                <span v-if="!isSubmitting">Update asset</span>
...
import { assetsService, type Asset, type UpdateAssetData } from '@/services/assets'
...
const assetId = ref<string>(route.params.id as string)
...
const tabs: Tab[] = [
  ...
  },
  {
    ...
    fields: ['department', 'location_id', 'employee_id'],
  },
  {
    id: 'additional-info',
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (photoSource.value === 'upload') {
      if (hasNewPhoto.value && photoFile.value) {
        assetData.photo = photoFile.value
        // Clear remote URL if switching to upload
        if (assetData.custom_fields) delete assetData.custom_fields.photo_url
      }
    } else {
      // Remote URL mode
      if (form.photo_url) {
    ...
    const updatedAsset = await assetsService.update(assetId.value, assetData)

    successMessage.value = `Asset "${updatedAsset.name}" has been updated successfully.`
...
const loadAsset = async () => {
  ...
  try {
    const asset = await assetsService.getById(assetId.value)
    ...
    form.purchase_price = asset.purchase_price?.toString() || ''
    form.vendor = asset.vendor || ''
    form.department = asset.department || ''
    form.location_id = asset.location_id?.toString() || ''
    form.employee_id = asset.employee_id?.toString() || ''
    form.warranty_expiration = asset.warranty_expiration || ''
    form.condition = asset.condition || ''
    form.notes = asset.notes || ''
    form.custom_fields = asset.custom_fields || {}
    
    // Handle photo source
    if (asset.custom_fields?.photo_url) {
      form.photo_url = asset.custom_fields.photo_url
      photoSource.value = 'url'
    } else if (asset.photo_url) {
      // Assuming asset.photo_url comes from file upload (via backend URL generation)
      photoPreview.value = asset.photo_url
