    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl font-bold text-primary-dark">Edit employee</h1>
            <p class="text-slate-500 mt-1">Update employee details</p>
          </div>
      ...
      <div v-if="isLoading" class="bg-white border border-border-light rounded-2xl p-8 text-center">
        ...
        <p class="text-slate-600">Loading employee...</p>
      </div>
...
import { employeesService, type UpdateEmployeeData } from '@/services/employees'
...
const loadEmployee = async () => {
  ...
  try {
    const employee = await employeesService.getById(employeeId.value)
    ...
    form.work_location_type = employee.work_location_type || ''
    form.notes = employee.notes || ''
    form.custom_fields = employee.custom_fields || {}
    
    // Handle photo initialization
    if (employee.custom_fields?.photo_url) {
      form.photo_url = employee.custom_fields.photo_url
      photoSource.value = 'url'
    } else {
      // Assuming no direct photo_url field on employee yet, but if there was, handle here
      // For now we default to upload if no remote URL
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (photoSource.value === 'upload' && photoFile.value) {
      ...
      // If switching to upload, you might want to clear the photo_url from custom_fields
      // if (employeeData.custom_fields) delete employeeData.custom_fields.photo_url
    ...
    const updatedEmployee = await employeesService.update(employeeId.value, employeeData)
    successMessage.value = `Employee "${updatedEmployee.first_name} ${updatedEmployee.last_name}" updated successfully.`
